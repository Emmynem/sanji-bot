...
const buyConfirmation = async (ctx: MyContext) => {
	await ctx.answerCbQuery(); // Remove loading animation

	if (ctx.session.user) {
		if (ctx.session.user.settings) {
			ctx.session.user.settings.buyConfirmation = ctx.session.user.settings.buyConfirmation ? false : true;
		}
	}
	...
	try {
		...
		if (ctx.session.user) {
			if (ctx.session.user.settings) {
				ctx.session.user.settings.buyConfirmation ? 
					await ctx.reply(
						`Buy confirmation is enabled. You will be asked to confirm before buying tokens.`
					) :
					await ctx.reply(
						`Buy confirmation is disabled. You will *NOT* be asked to confirm before buying tokens.`
					)	; 
			}
		}
	} catch (error) {
