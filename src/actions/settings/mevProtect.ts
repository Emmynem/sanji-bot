...
const mevProtect = async (ctx: MyContext) => {
	await ctx.answerCbQuery(); // Remove loading animation

	if (ctx.session.user) {
		if (ctx.session.user.settings.mevProtectValue) {
			ctx.session.user.settings.mevProtectValue = ctx.session.user.settings.mevProtectValue === "Turbo" ? "Secure" : "Turbo";
		}
	}
