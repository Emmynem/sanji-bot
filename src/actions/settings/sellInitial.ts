...
const sellInitial = async (ctx: MyContext) => {
	await ctx.answerCbQuery(); // Remove loading animation

	if (ctx.session.user) {
		if (ctx.session.user.settings) {
			ctx.session.user.settings.sellInitialStatus = ctx.session.user.settings.sellInitialStatus ? false : true;
		}
	}
