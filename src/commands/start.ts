...
import { sanjiURL, newUserDetails, botLink, escapeMarkdownV2, forwardUserWalletBalanceUpdated, getSession, getSolanaPrice } from "../config";
...
const start = async (ctx: MyContext) => {
	...
	if (ctx.session.user) {
		const response = await getSolanaPrice();
		if (!response.err) ctx.session.user.solanaPrice = response.data.solana.usd;
