...
const paperTradingMode = async (ctx: MyContext) => {
	await ctx.answerCbQuery(); // Remove loading animation

	if (ctx.session.user) {
		if (ctx.session.user.settings) {
			ctx.session.user.settings.paperTradingMode = ctx.session.user.settings.paperTradingMode ? false : true;
		}
	}
	...
	try {
		...
		if (ctx.session.user) {
			if (ctx.session.user.settings) {
				ctx.session.user.settings.paperTradingMode ? 
					await ctx.reply(
						`Paper Trading Mode enabled! All swaps will be simulated.`
					) :
					await ctx.reply(
						`Paper Trading Mode disabled. Real swaps will be executed.`
					); 
			}
		}
	} catch (error) {
